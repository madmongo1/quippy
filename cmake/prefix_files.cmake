...
if (defined.function.prefix_files VERSION_LESS "${THIS_VERSION}")
    ...
    function(prefix_files outvar)
        set(options)
        set(oneValueArgs PREFIX APPEND BASE)
        ...
        foreach (file ${ARG_FILES})
            set(binary "${CMAKE_CURRENT_BINARY_DIR}/${ARG_PREFIX}/${file}")
            set(source "${CMAKE_CURRENT_SOURCE_DIR}/${ARG_PREFIX}/${file}")
            if (EXISTS "${binary}")
                set(prefixed_file "${binary}")
                list(APPEND result "${binary}")
            elseif (EXISTS "${source}")
                set(prefixed_file "${source}")
                list(APPEND result "${source}")
            else ()
                message(FATAL_ERROR "cannot find ${ARG_PREFIX}/${file}")
            endif ()
